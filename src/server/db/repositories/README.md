...
| `config.ts` | `config_plataforma`, `config_historico`, `tipos_moeda` (024) | configuração por upsert (`$2::jsonb` com `JSON.stringify`) · **histórico só inserir** · tipo de moeda inserir e atualizar; `garantirTiposMoeda` só semeia tabela vazia. **Leitura de `jsonb` sai como `valor::text`** e passa por `JSON.parse`: o driver devolve string `jsonb` já decodificada, e "0,5" e `"0,5"` ficariam indistinguíveis |
| `caixas.ts` | `caixas` (025) | inserir e atualizar; `buscarCaixa` com `FOR UPDATE` para a edição concorrente |
