...
- **C-1** 🟡 — Aplicação da migration `011_retiradas.sql` no Supabase via `npm run db:migrate`.
- **C-2** 🟡 — Aplicação da migration `012_retiradas_ledger.sql` no Supabase via `npm run db:migrate`.
...
- **`src/server/db/migrations/012_retiradas_ledger.sql`**: Migration ajustando a constraint `CHECK` de `aurea.ledger_entries.tipo` para incluir `'taxa_retirada'`.
...
- **C-1** 🟡 — Aplicação da migration `011_retiradas.sql` no banco Supabase.
- **C-2** 🟡 — Aplicação da migration `012_retiradas_ledger.sql` no banco Supabase (atualização da constraint CHECK do ledger para aceitar `taxa_retirada`).
...
### Banco de Dados
- **`src/server/db/migrations/011_retiradas.sql`**:
