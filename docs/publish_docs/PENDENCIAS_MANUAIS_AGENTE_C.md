...
| | |
|---|---|
| **O que falta** | Executar a migration `011_retiradas.sql` no banco Supabase |
...
| | |
|---|---|
| **O que falta** | Executar a migration `012_retiradas_ledger.sql` no banco Supabase (atualiza constraint CHECK do ledger para incluir `'taxa_retirada'`) |
| **Quem pode fazer** | **Gabriel** (via terminal ou SQL Editor do Supabase) |
